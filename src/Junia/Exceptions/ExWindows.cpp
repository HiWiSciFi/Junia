 ...
 *
 * @file      ExWindows.cpp
 * @brief     Contains the ExWindows class implementation
 * @author    Max Hager
 * @date      2024-07-10
...
ExWindows::ExWindows(unsigned long code, std::exception_ptr previous, CodePos location) noexcept
	...
	if (LocalFree(buf) == NULL) return;

	std::stringstream ss;
	ss << u8"\n\nERROR: Failed to free buffer for error message. Win32 Error Code: 0x"
	   << std::setfill('0')
	   << std::setw(4) // error codes are only up to 15999 = 0x3E7F
	   << std::right
	   << std::hex
	   << GetLastError()
	   << std::endl;
